...
try:
    ...
    # Also support putting the key in a local file: ml-service/openai_key.txt
    if not _OPENAI_API_KEY:
        try:
            key_path = os.path.join(os.path.dirname(__file__), "openai_key.txt")
            with open(key_path, "r", encoding="utf-8") as f:
                _OPENAI_API_KEY = f.read().strip()
        except Exception:
...
def call_ollama_chat(message: str) -> Optional[str]:
    """
    Try to get a response from a local Ollama model.
    Requires Ollama to be running on http://localhost:11434
    and a model name (default: llama3.2) to be available.
    """
        ...
        resp = requests.post(
            ...
            json={
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a helpful, general-purpose AI assistant integrated into a resume analyzer app.",
                    },
